...
loader = machinery.SourceFileLoader('forming_data_for_translation_script.py',
                                    str(Path.cwd() /'scripts'/'functions'/ \
                                        'forming_data_for_translation_script.py'))
...
for target_language_tag in args.target_langs_tags.split(','):
    for column in ['context', 'question', 'answer']:
        data_column = form_data.forming_column_for_translation(data[column],
                                                               args.source_lang_tag,
                                                               target_language_tag)

        path = Path(args.output_dir_path) / \
                    Path(args.input_data_path).stem / target_language_tag / f'{column}s'
        path.mkdir(parents=True, exist_ok=True)

        pd.DataFrame(data_column).to_json(f'{str(path)}/data.json',
