...
def main():
    ...
    forced_bos_token_id = (tokenizer.convert_tokens_to_ids(data_args.forced_bos_token) 
                           if data_args.forced_bos_token is not None
                            else tokenizer.convert_tokens_to_ids(data_args.target_lang))
    ...
    def preprocess_function(examples):
        ...
        if len(''.join(targets)) > 0:
            ...
            targets = ['' for i in inputs_by_sent_cons]
    ...
    def find_num_sent_in_inputs(examples):
        ...
        return num_sent_in_inputs
    
    if training_args.do_train:
        if "train.0" not in raw_datasets:
    ...
    num_sent_in_inputs = find_num_sent_in_inputs(raw_datasets["test"])
    ...
    def merge_sent(translated_sent, num_sent_in_inputs):

        start_count = 0
        translations = []
        for num in num_sent_in_inputs:
            translations += [' '.join(translated_sent[start_count:start_count+num])]
            start_count += num
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                num_projections = len([pred for pred in predictions if re.findall('\[.+?\]', pred)])
