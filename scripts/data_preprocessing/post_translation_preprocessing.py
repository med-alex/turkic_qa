...
initial_data_without_brackets = data[~data.context.str.contains('\[|\]')]
initial_data_with_brackets = data[data.context.str.contains('\[|\]')]

data.context = data.apply(lambda data: prep.find_answer_in_contex(data.context, data.answer) \
                            if not re.findall('\[.+\]', data.context) else data.context, axis=1)
data['answer_start'] = data.context.apply(lambda text: text.find('[') if re.findall('\[.+\]', text) else -1)
...
if args.first_file_sample_size is not None \
    ...
    first_file_data = data[data.index.isin(initial_data_with_brackets.index)].sample(args.first_file_sample_size, random_state=42)
    second_file_data = data[(~data.index.isin(first_file_data.index)) & (data.index.isin(initial_data_with_brackets.index))]\
...
with open(info_path, 'w', encoding="utf-8") as writer:
    ...
    writer.write(f'initial_num_rows_without_brackets: {len(initial_data_without_brackets.index)}'+'\n')
    writer.write(f'initial_perc_of_rows_with_brackets: {round((len(initial_data_with_brackets.index) / initial_data_len) * 100, 2)}'+'\n')
    writer.write(f'initial_perc_of_rows_without_brackets: {round((len(initial_data_without_brackets.index) / initial_data_len) * 100, 2)}'+'\n')
    ...
    writer.write(f'prep_perc_of_rows_without_brackets: {round((len(prep_data_without_brackets.index) / len(data.index)) * 100, 2)}'+'\n')
