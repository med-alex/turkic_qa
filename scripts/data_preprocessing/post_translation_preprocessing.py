...
existing_one_or_multiple_found_answers = len(data.found_left_brackets) == 1 \
                                        and len(data.found_right_brackets) == 1 \
                                        and len(data.found_answer) > 0 \
                                        and len(data.found_answer[0]) > 2
one_anomaly_short_found_answer = len(data.found_left_brackets) == 1 \
                                and len(data.found_right_brackets) == 1 \
                                and len(data.found_answer) > 0 \
                                and len(data.found_answer[0]) <= 2
no_found_answer = len(data.found_left_brackets) == 0 \
                and len(data.found_right_brackets) == 0

data.context = data.apply(lambda data: prep.answer_in_context_validation(data.context, \
                                                                         data.answer, \
                                                                         data.found_answer[0][1:-1]) \
                                if existing_one_or_multiple_found_answers \
                                    else prep.find_answer_in_context_from_translated_answer(data.context, \
                                                                                            data.answer) \
                                    if one_anomaly_short_found_answer \
...
data['answers'] = data.apply(lambda data: {'text': np.array([data.answer], dtype=object), \
                             ...
                             axis=1)
data = data.drop(columns=['answer', 'answer_start', 'found_answer',
...
if three_files_condition:
    first_file_data = data[data.index.isin(initial_data_with_correct_brackets.index)] \
                            .sample(args.first_file_sample_size, random_state=42)
    second_file_data = data[(~data.index.isin(first_file_data.index)) \
                                & (data.index.isin(initial_data_with_correct_brackets.index))] \
...
with open(info_path, 'w', encoding="utf-8") as writer:
    writer.write(f'initial_num_rows: \
                    {initial_data_len}' + '\n\n')
    writer.write(f'initial_num_rows_with_any_brackets: \
                    {len(initial_data_with_any_brackets.index)}' + '\n')
    writer.write(f'initial_num_rows_without_any_brackets: \
                    {len(initial_data_without_any_brackets.index)}' + '\n')
    writer.write(f'initial_perc_of_rows_with_any_brackets: \
                    {round((len(initial_data_with_any_brackets.index) / initial_data_len) * 100, 2)}' + '\n')
    writer.write(f'initial_perc_of_rows_without_any_brackets: \
                    {round((len(initial_data_without_any_brackets.index) / initial_data_len) * 100, 2)}' + '\n\n')
    writer.write(f'initial_num_rows_with_correct_brackets: \
                    {len(initial_data_with_correct_brackets.index)}' + '\n')
    writer.write(f'initial_num_rows_without_correct_brackets: \
                    {len(initial_data_without_correct_brackets.index)}' + '\n')
    writer.write(f'initial_perc_of_rows_with_correct_brackets: \
                    {round((len(initial_data_with_correct_brackets.index) / initial_data_len) * 100, 2)}' + '\n')
    writer.write(f'initial_perc_of_rows_without_correct_brackets: \
                    {round((len(initial_data_without_correct_brackets.index) / initial_data_len) * 100, 2)}' + '\n\n')
    writer.write(f'prep_num_rows: {len(data.index)}' + '\n')
    writer.write(f'prep_num_rows_with_brackets: \
                    {len(prep_data_with_brackets.index)}' + '\n')
    writer.write(f'prep_num_rows_without_brackets: \
                    {len(prep_data_without_brackets.index)}' + '\n')
    writer.write(f'prep_perc_of_rows_with_brackets: \
                    {round((len(prep_data_with_brackets.index) / len(data.index)) * 100, 2)}' + '\n\n')
    writer.write(f'thrown_rows_without_brackets_to_initial_data: \
                    {round((len(prep_data_without_brackets.index) / initial_data_len) * 100, 2)}' + '\n\n')

    if three_files_condition:
        writer.write(f'first_file_size: {first_file_data.shape}, \
                        split: {Path(args.output_data_path).stem}' + '\n')
        writer.write(f'second_file_size: {second_file_data.shape}, \
                        split: {Path(args.second_output_data_path).stem}' + '\n')
        writer.write(f'second_file_size: {third_file_data.shape}, \
                        split: {Path(args.third_output_data_path).stem}' + '\n')
        ...
        writer.write(f'first_file_size: {data.shape}, \
                        split: {Path(args.output_data_path).stem}' + '\n')
