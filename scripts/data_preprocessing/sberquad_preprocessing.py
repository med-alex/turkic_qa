...
if 'answers.text' not in data.columns and 'answer.start' not in data.columns:
    data['answers.text'] = data.answers.apply(lambda answer_info: \
                                                answer_info['text'][0])
    data['answers.answer_start'] = data.answers.apply(lambda answer_info: \
                                                        int(answer_info['answer_start'][0]))
    data = data.drop(columns=['answers'])

data = data.rename(columns={'answers.text': 'answer', \
...
if args.sample_size is not None and len(data.index) >= args.sample_size:
    ...
    raise BaseException(f"Existing {len(data.index)} clean data rows is not enough \
                            to sample desired {args.sample_size} rows")
